...
export default function PaymentsTab({ search }: { search: string }) {
  ...
  const selectedPayment = response?.data.find(x => x.id === selectedPaymentId);
...
function PaymentDetailsModal({ payment, onClose }: PaymentDetailsModalProps) {
  const statusColors = {
    ...
    LATE: 'text-orange-400 bg-orange-500/10 border-orange-500/20',
      ...
      >
        ...
        <div className="bg-slate-900 px-6 py-5 text-white flex items-center justify-between relative overflow-hidden">
          ...
          <div className="relative z-10 flex-1 min-w-0 pr-4">
            ...
            <p className="text-[10px] text-slate-400 font-mono">ID: {payment.id}</p>
          </div>
          <div className="relative z-10 shrink-0">
            <span className={`text-xs font-bold px-3 py-1 rounded-full border shadow-sm ${statusColors[payment.status as keyof typeof statusColors] || 'text-slate-400 bg-slate-500/10 border-slate-500/20'}`}>
        ...
        <div className="grid grid-cols-2 gap-3 p-4 bg-slate-50 border-b border-slate-100">
          <div className="bg-white border border-slate-100 rounded-2xl p-3 flex items-center gap-3 shadow-sm">
            ...
            <div>
              ...
              <p className="text-sm font-extrabold text-slate-900 leading-none">£{Number(payment.amount).toLocaleString()}</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-3 flex items-center gap-3 shadow-sm">
            ...
            <div>
              ...
              <p className="text-sm font-extrabold text-slate-900 leading-none">Round {payment.round.roundNumber}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm space-y-3">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
              <User size={14} className="text-slate-400" /> Participant Info
            </h4>
            <div className="space-y-2 text-sm">
              <div>
                <span className="text-xs text-slate-400">Full Name</span>
                <p className="font-semibold text-slate-800 mt-0.5">{payment.participant.fullName}</p>
              </div>
              <div>
                ...
                <p className="font-semibold text-slate-800 mt-0.5 flex items-center gap-1.5">
                  <Mail size={12} className="text-slate-400" /> {payment.participant.email}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm space-y-3">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
              <Calendar size={14} className="text-slate-400" /> Payment Info
            </h4>
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div className="col-span-2">
                <span className="text-xs text-slate-400">Pardna Fund</span>
                <p className="font-semibold text-slate-800 mt-0.5">Weekend Trip Fund</p>
              </div>
            </div>
          </div>
        </div>
