...
export default function AdminSidebar({ collapsed, onToggle }: AdminSidebarProps) {
  ...
  return (
    ...
    <aside
      id="admin-sidebar"
      className={cn(
        "fixed left-0 top-0 bottom-0 flex flex-col bg-[var(--sidebar-bg)] text-white z-50 transition-all duration-300 ease-in-out border-r border-white/5",
        "w-60",
        collapsed
    ...
    >
      {/* Logo Header */}
      <div
        className="flex items-center justify-between border-b border-white/5 w-full"
      ...
      >
        <img
          ...
          alt="PardnaBook logo"
          className={cn(
            "object-contain transition-all duration-300 h-8",
            collapsed ? "w-8 md:w-8" : "w-[148px]"
          )}
        />
        {!collapsed && (
          <button
            onClick={onToggle}
            className="md:hidden flex items-center justify-center p-1.5 rounded-lg text-white/80 hover:text-white hover:bg-white/10 transition-colors border-none bg-transparent cursor-pointer"
            title="Close Sidebar"
          >
      ...
      <button
        ...
        className="absolute top-[50px] -right-4 z-50 hidden md:flex items-center justify-center w-9 h-9 rounded-full bg-[var(--color-primary)] text-white shadow-lg hover:bg-[var(--color-primary-light)] hover:scale-110 active:scale-95 transition-all duration-200 cursor-pointer border-none outline-none"
      >
      ...
      <div className="px-2 py-4 border-t border-white/5 flex flex-col gap-2">
        {/* Banker Dashboard NavLink */}
        <NavLink
          to="/dashboard"
          title={collapsed ? 'Banker Dashboard' : undefined}
          className={cn(
            'flex items-center w-full rounded-[10px] text-sm font-medium text-gray-400 hover:text-white hover:bg-white/5 transition-all cursor-pointer bg-transparent border-none overflow-hidden no-underline',
            collapsed ? 'justify-center px-0 py-3' : 'gap-3 px-4 py-3'
          )}
        >
